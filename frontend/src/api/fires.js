...
// Tunable multipliers on the alert-zone *buffer* (the ring outside the fire
// polygon, not the fire perimeter itself). These scale how far the prediction
// ring extends past the burning footprint along the wind axis. Vertices in
// between interpolate smoothly, so crosswind sides sit at the average.
// 1.0 = plain uniform radius. Tweak these to taste.
const HEAD_EXTEND_MULT = 1.2; // downwind (head) buffer multiplier
const BACK_EXTEND_MULT = 0.5; // upwind (heel) buffer multiplier
...
function lengthToBreadth(windMph) {
  ...
  const lb =
    0.936 * Math.exp(0.2566 * u) + 0.461 * Math.exp(-0.1548 * u) - 0.397;
  ...
  const cap =
    u > WINDY_THRESHOLD_MPH ? ELLIPSE_LB_MAX_WINDY : ELLIPSE_LB_MAX_CALM;
...
function predictedPerimeter(props) {
  ...
  const windFromDeg = Number.isFinite(Number(props.wind_direction_deg))
    ? Number(props.wind_direction_deg)
    : 270;
  ...
  const degPerKmLon =
    1.0 / (DEG_LAT_KM * Math.max(Math.cos((lat * Math.PI) / 180), 0.01));
  ...
  const spotBearings = Array.from(
    { length: SPOT_COUNT },
    (_, k) => Math.PI / 4 + (Math.PI / 2) * ((seed * (k + 3.1)) % 1.0),
  );
  ...
  for (let i = 0; i <= ELLIPSE_VERTICES; i++) {
    ...
    let r =
      1 + warp * NOISE_AMPLITUDE * (HEAD_BIAS_MIN + HEAD_BIAS_MAX * headBias);
    ...
    for (let k = 0; k < spotBearings.length; k++) {
      ...
      r +=
        SPOT_AMPLITUDE *
        Math.exp(-(d * d) / (2 * SPOT_WIDTH_RAD * SPOT_WIDTH_RAD));
    }
    ...
    ring.push([
      centerLon + eastKm * degPerKmLon,
      centerLat + northKm * degPerKmLat,
    ]);
  }
  return { type: "Polygon", coordinates: [ring] };
}
...
function ensurePolygonGeometry(feature) {
  ...
  if (!point) {
    ...
    return {
      ...
      properties: {
        ...p,
        centroid: p.centroid || polygonCentroid(feature.geometry),
      },
    };
  }
  ...
  if (ellipse) {
    return {
      ...
      properties: {
        ...p,
        centroid: point,
        perimeter_source: p.perimeter_source || "predicted",
      },
    };
  }
  return {
...
// When wind is provided, the per-vertex buffer is scaled between
// BACK_EXTEND_MULT (pure upwind) and HEAD_EXTEND_MULT (pure downwind) using a
// smooth cosine interpolation, so the prediction ring bulges in the downwind
// direction without touching the underlying fire polygon.
function offsetPolygonOutward(geometry, centroid, bufferKm, windFromDeg) {
  const ring = geometry?.coordinates?.[0];
  if (!ring?.length || !centroid) return null;
  const [cLon, cLat] = centroid;
  const degPerKmLat = 1.0 / DEG_LAT_KM;
  const degPerKmLon =
    1.0 / (DEG_LAT_KM * Math.max(Math.cos((cLat * Math.PI) / 180), 0.01));
  const hasWind = Number.isFinite(Number(windFromDeg));
  const spreadBearingRad = hasWind
    ? (((Number(windFromDeg) + 180) % 360) * Math.PI) / 180
    : 0;
  const offset = ring.map(([lon, lat]) => {
    ...
    let buf = bufferKm;
    if (hasWind) {
      // Vertex compass bearing (east = sin, north = cos), compared against
      // the fire's spread bearing. align ∈ [-1 upwind, +1 downwind].
      const vertBearingRad = Math.atan2(dxKm, dyKm);
      const align = Math.cos(vertBearingRad - spreadBearingRad);
      // Map align from [-1, +1] → [BACK_EXTEND_MULT, HEAD_EXTEND_MULT].
      const mix = (align + 1) / 2;
      const mult =
        BACK_EXTEND_MULT + (HEAD_EXTEND_MULT - BACK_EXTEND_MULT) * mix;
      buf = bufferKm * mult;
    }
    const scale = (dist + buf) / dist;
    return [
      cLon + dxKm * scale * degPerKmLon,
      cLat + dyKm * scale * degPerKmLat,
    ];
  });
...
export function buildAlertZones(fireCollection) {
  const features = (fireCollection?.features || [])
    .map((f) => {
      ...
      // Fully-contained fires render green on the map — no active spread means
      // no outward prediction risk, so we suppress the alert-zone halo too.
      if (Number(p.containment_pct) >= 100) return null;
...
async function fetchHybrid() {
  ...
  const [live, mock] = await Promise.all([
    fetchLive().catch((e) => {
      console.warn("live fires failed:", e);
      return empty;
    }),
    fetchMock().catch((e) => {
      console.warn("mock fires failed:", e);
      return empty;
    }),
  ]);
  const seen = new Set();
